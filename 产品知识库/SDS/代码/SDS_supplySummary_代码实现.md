...
- **更新时间**：2026-04-02（基于 `scp-foundation` 源码复核，DAO XML 仍以依赖 jar 为准）
- **本次调研范围**：sds/supplySummary/ 功能全链路（Controller、Service、DAO、枚举、数据流、SQL 实现）
- **代码来源**：scp-foundation 本地仓库 + scp-sds-infra-base jar（编译包；工作区内无 `SupplySummaryBasicDao.xml` 源文件）
...
当前实现覆盖 **6 类供应数据来源**（`getTableNames` 与 `SupplyTypeEnum` 与之一致）：采购订单、运输订单、制造订单、期初库存、采购在途、**通用供应**（表 `sds_supply_universal`）。计划/非计划/全部三类 `supplyCategory` 下均会把 `sds_supply_universal` 纳入参与 `information_schema` 公共维度计算的表集合。
...
- 接口入参 `supplyVersionId` 在 `SupplySummaryServiceImpl.selectPageInfo` 中**未使用**（仅 Controller 透传），若产品语义依赖「供应版本」过滤，需确认是否应在 DAO 条件或 `queryCriteriaParam` 中落地。
- `page` 与 `dynamicTimeSeries` 对时段处理不一致：`page` 内对 `periodEnd` 做了 `+1` 天再与明细 `supply_date` 做 `[start, end)` 匹配；`selectDynamicTimeSeries` **未**做该平移，前端若两套接口拼表头与数据，需注意边界是否一致。

### 4.2.5 dimensionDropdown 逻辑

1. `initDimensionList()` 始终返回 8 个**硬编码固定维度**：物品代码、物品名称、物品类型、库存点代码、库存点名称、物品系列代码、物品系列名称、供应子类型
2. 查询 `information_schema.COLUMNS`，找出在**所有涉及表中均存在**的 varchar 字段（HAVING count = tableCount），追加为额外公共维度（`selectGroupFields`，表集合含 `sds_supply_universal`）
3. 若公共字段中不包含 `order_flag`，则追加「订单标识」维度 —— 实现里用 `existFields.contains("orderFlag")` 判断，而 `existFields` 来自 SQL 返回的下划线列名，**与 camelCase 的 `orderFlag` 不匹配**，逻辑仍与原文档 4.6 问题三一致
...
每条 SQL 均为 **多段 UNION ALL 硬编码**（在原有 5 张业务表基础上已扩展 **通用供应表** 分支，具体段数以 jar 内 XML 为准），每段对应供应相关表，并 JOIN MDS 的以下主数据表：
...
| `sds_ord_work_order` | 制造订单 | supply_date, quantity, order_flag | order_flag='NO' → 计划供应；order_flag='YES' → 非计划供应 |
| `sds_ord_transport_order` | 运输订单 | supply_date, quantity, order_flag | order_flag='NO' → 计划供应；order_flag='YES' → 非计划供应 |
| `sds_sup_boh_stock` | 期初库存 | quantity | 仅属于非计划供应，无 order_flag 过滤 |
| `sds_sup_purchase_forecast_stock` | 采购在途 | supply_date, quantity | 仅属于非计划供应 |
| `sds_supply_universal` | 通用供应 | （以 jar 内 SQL/实体为准） | 计划/非计划/全部三类均参与维度与汇总（具体 plan/non-plan 规则以 SQL 为准） |

### 供应分类与表的映射关系（Service 层 `getTableNames`）

| supplyCategory | 参与维度探测与汇总的表集合（代码） |
|----------------|--------------------------------------|
| PLAN_SUPPLY | sds_ord_purchase_order、sds_ord_work_order、sds_ord_transport_order、sds_supply_universal |
| NON_PLAN_SUPPLY | sds_sup_purchase_forecast_stock、sds_sup_boh_stock、sds_ord_work_order、sds_ord_transport_order、sds_supply_universal |
| ALL（空） | 上述 6 张表全部 |

**说明**：各分类下 `order_flag` 等对计划/非计划的切分仍以 **DAO 中 UNION 各段 WHERE** 为准；上表仅反映 Java 侧纳入的物理表名。
...
- 已具备完整的供应汇总查询链路，覆盖 6 类供应数据源（含通用供应）
...
新增一类供应表需同时修改以下多处，且每处改动均为侵入式：

1. **ServiceImpl**：`getTableNames()` 中追加硬编码表名；`translate()` 的 ImmutableMap 中按需追加枚举映射
2. **DAO XML（6 条 SQL）**：每条 UNION ALL 分支均需添加新表的查询段（条数 × 分支数）
3. **SupplyTypeEnum**：新增枚举值（若需在界面展示供应子类型文案）
...
### 问题五：translate() 无法匹配的枚举 code（已缓解）

- **历史问题**：空列表时 `get(0)` 曾导致 `IndexOutOfBoundsException`。
- **当前代码**（`SupplySummaryServiceImpl.translate`）：当 `targetCommonEnums` 为空时 `return`，**不再**对 `get(0)` 取值，避免崩溃；无法匹配的字段保持原 code 展示。

**残留影响**：旧数据或未知 code 仍可能以原始 code 出现在界面上，属展示一致性问题而非宕机。
