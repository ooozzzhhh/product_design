...
const FunctionDetailView: React.FC<Props> = ({
  ...
  const allFilteredHistory = useMemo(() => {
    return runHistory.filter((r) => r.functionId === func.id).filter((run) => {
      const matchesTaskId = !taskIdSearch || run.id.toLowerCase().includes(taskIdSearch.toLowerCase());
      const matchesOperator = !operatorSearch || run.operator.toLowerCase().includes(operatorSearch.toLowerCase());
      ...
      return matchesTaskId && matchesOperator && matchesStatus && matchesTime;
    });
  }, [runHistory, func.id, taskIdSearch, operatorSearch, statusFilter, startDate, endDate]);
  ...
  const handleSearch = () => {
    setTaskIdSearch(taskIdInput);
    setOperatorSearch(operatorInput);
    setCurrentPage(1);
  };
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <select
              value={func.id}
              onChange={(e) => {
                const newFunc = functions.find(f => f.id === e.target.value);
                if (newFunc) onFunctionChange(newFunc);
              }}
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex-1 min-w-[180px]">
            <input type="text" placeholder="任务ID" className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-sm focus:ring-2 focus:ring-indigo-500/20" value={taskIdInput} onChange={(e) => setTaskIdInput(e.target.value)} />
          </div>
          <div className="flex-1 min-w-[180px]">
            <input type="text" placeholder="操作人" className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-sm focus:ring-2 focus:ring-indigo-500/20" value={operatorInput} onChange={(e) => setOperatorInput(e.target.value)} />
          </div>
          <button onClick={handleSearch} className="px-4 py-2 bg-[#1a1f71] text-white rounded-lg text-sm font-bold hover:bg-[#121650]">查询</button>
