...
* **库存成本**（`cost_category = INVENTORY`）
  ...
  * **单位最大库存违反成本**：库存超过上限时按件计的违反成本；落表时 `metric_type = UNIT`，`cost_sub_category = VIOLATION`。

  * **库存持有成本**：与在库持有相关的按件成本；落表时 `metric_type = UNIT`，`cost_sub_category = HOLDING`。

  * **资金占用成本率（年化）**：资金占用的年化费率；落表时 `metric_type = RATE`，`cost_sub_category = CAPITAL_OCCUPATION`。
...
* 如何实现可扩展？核心在于「维度固定 + 枚举扩展」这两个点：
  ...
  * 行表通过枚举来表达“是什么成本 + 针对谁”：
    ...
    * metric\_type 表示计算方式：UNIT（按件）、FIXED（按次）、RATE（按率）。cost\_sub\_category 表示业务子类型，可空，例如：STORAGE（仓储成本率）、CAPITAL\_OCCUPATION、LATE\_PENALTY、NON\_DELIVERY。将来能耗里需要“电费单价/固定电费/碳排放成本”，则 metric\_type 用 UNIT/FIXED/RATE，cost\_sub\_category 新增 ENERGY\_UNIT、ENERGY\_FIXED、CARBON 等。
...
|---------------|-------------|-------------------|----------|
| PRODUCTION | UNIT | (空) | 单位生产成本 |
| PRODUCTION | FIXED | (空) | 固定生产成本 |
| PURCHASE | UNIT | (空) | 单位采购成本 |
| PURCHASE | FIXED | (空) | 固定采购成本 |
| TRANSPORT | UNIT | (空) | 单位运输成本 |
| TRANSPORT | FIXED | (空) | 固定运输成本 |
| INVENTORY | RATE | STORAGE | 仓储成本率（年化） |
| INVENTORY | RATE | CAPITAL_OCCUPATION | 资金成本率（年化） |
...
3. 查询全局参数，获取**默认仓储成本率（年化）**

笛卡尔积，1 X 2，仓储成本率默认填充全局参数值，形成库存成本原始数据
...
3. 查询全局参数，获取**默认资金成本率（年化）**

笛卡尔积，1 X 2，资金成本率默认填充全局参数值，形成库存成本原始数据
...
3. 若全局参数里“库存点物品的仓储成本率=启用”，检查仓储成本率（年化）是否有0值/空值，有一个就记录**错误数+1**

4. 若全局参数里”库存点物品的库存资金占用成本=启用”，检查资金成本率是否有空值，有一个空值就记录**错误数+1**

5. 除了备注、仓储成本率（年化）、资金成本率，检查当前数据是否有其它字段值为空，有一个空值就记录**错误数+1**
...
按库存价值与仓储成本率（年化）计费，与库存资金占用成本的计量结构对齐。

计算公式：`仓储成本 = 平均库存价值 × 仓储成本率（年化） × 时间系数`

- **平均库存价值**：与下文「库存资金占用成本」相同，`(期初库存 + 期末库存) / 2 × 单位价值`
- **仓储成本率（年化）**：优先使用成本明细中 `metric_type='RATE'` + `cost_sub_category='STORAGE'` 的 cost_value，若无则使用全局参数「默认仓储成本率（年化）」（百分比口径与资金成本率（年化）一致）
- **单位价值获取优先级**：与下文「库存资金占用成本」一致
- **时间系数**：与下文「库存资金占用成本」一致（系统时段为月：1/12；周：1/52；日：1/365）

**8. CAPITAL_OCCUPATION_COST（库存资金占用成本）**

按库存价值和资金成本率（年化）计费，反映库存占用资金的机会成本。

计算公式：`库存资金占用成本 = 平均库存价值 × 资金成本率（年化） × 时间系数`

- **平均库存价值**：`(期初库存 + 期末库存) / 2 × 单位价值`
- **资金成本率（年化）**：优先使用成本明细中的值，若无则使用全局参数「默认资金成本率（年化）」
- **单位价值获取优先级**：
...
示例：资金成本率=5%（年化），平均库存=100件，单位价值=50元，系统时段为月 → 100 × 50 × 5% × (1/12) = 20.83元
...
1. **全局参数**：「库存点物品的库存持有成本」调整为「库存点物品的仓储成本率」；去除「库存点物品的最大库存违反成本」开关；新增「**默认仓储成本率（年化）**」数值参数（与「**默认资金成本率（年化）**」并列，百分比、默认 0%，默认 0% 时建议弹窗提示配置）；保留「库存点物品的库存资金占用成本」开关。
2. **成本明细-库存成本TAB**：展示「仓储成本率（年化）」「资金成本率（年化）」；**库存成本合计（率）**为两者之和，**仅界面展示**（不在 PRD 正文展开公式，详见原型图 `财务管理.html`）
...
* **数据表格**（按 Tab 不同）：
  ...
  * **库存成本**（挂靠对象：**库存点物品**）：勾选、物品代码、库存点、版本、时段、**仓储成本率（年化）**、**资金成本率（年化）**、货币、备注、操作；**库存成本合计（率）**为原型图中的只读汇总列。
...
* **弹窗**：
  ...
  * 新增/编辑弹窗：按当前 Tab 显示/隐藏不同字段 —— 生产含固定生产成本，采购含固定采购成本，运输含固定运输成本，库存成本含仓储成本率（年化）与资金成本率（年化），违约罚金含物品、库存点、客户、单位晚交罚金、单位欠交罚金。
...
* **行内编辑**：
  ...
  * 单位生产成本、固定生产成本、单位采购成本、固定采购成本、单位运输成本、固定运输成本、仓储成本率（年化）、资金成本率（年化）、单位晚交罚金、单位欠交罚金支持双击编辑。
