...
| 2026年03月14日 | 区卓挥     | 新增库存资金成本、优化初始化交互、新增成本合计列 |
| 2026年03月16日 | 区卓挥     | 成本明细行表：metric_type 拆分为 UNIT/FIXED/RATE，新增 cost_sub_category 存业务子类型 |
| 2026年03月23日 | 区卓挥     | 去除单位最大库存违反成本；库存持有成本调整为仓储成本率（RATE+STORAGE）；界面「库存成本合计（率）」见原型图 |
| 2026年03月23日 | 区卓挥     | 默认仓储/资金费率不再隐含「年化」：全局参数增加费率时间颗粒度（YEAR/MONTH）；成本明细行 RATE 落表须填 `time_cost_uom`；算法按颗粒度与系统时段计算时间系数 |
...
* 默认仓储费率时间颗粒度（下拉：`YEAR` 按年 / `MONTH` 按月；与默认仓储成本率组合含义：百分比对应「每年」或「每月」）

* 默认资金成本率（百分比，默认值 0%）

* 默认资金费率时间颗粒度（下拉：`YEAR` / `MONTH`；含义同上）
...
CREATE TABLE sop_pla_finance_price_line (
  id                      varchar(64)  PRIMARY KEY,
  header_id               varchar(64)  NOT NULL COMMENT '头表id',
...
CREATE TABLE sop_pla_finance_cost_line (
  ...
  metric_type     varchar(32) NOT NULL COMMENT '度量类型: UNIT/FIXED/RATE',
  cost_sub_category varchar(32) DEFAULT NULL COMMENT '成本子类: STORAGE/CAPITAL_OCCUPATION/LATE_PENALTY/NON_DELIVERY',
  ...
  time_cost_uom   varchar(32)   DEFAULT NULL COMMENT '时间成本单位：库存类 RATE 行必填 YEAR 或 MONTH；其它成本行可空，后续可扩展 DAY/WEEK 等',
  remark          varchar(255)  DEFAULT NULL,
  creator         varchar(64),
  create_time     datetime,
  modifier        varchar(64),
...
* **生产成本**（成本大类在表字段中为 `cost_category = PRODUCTION`）

  * **单位生产成本**：按**标准生产资源**计每件生产成本；落表时 `metric_type = UNIT`，维护 `standard_resource_id`，`cost_sub_category` 留空。

  * **固定生产成本**：按**标准生产资源**计每次固定生产成本；落表时 `metric_type = FIXED`，维护 `standard_resource_id`，`cost_sub_category` 留空。

* **采购成本**（`cost_category = PURCHASE`）

  * **单位或固定采购成本**：按**采购路径**维护单位成本或固定成本；落表时 `metric_type` 取 `UNIT` 或 `FIXED`，维护 `purchase_routing_id`，`cost_sub_category` 留空。

* **运输成本**（`cost_category = TRANSPORT`）

  * **单位或固定运输成本**：按**运输路径**及**来源库存点**维护单位或固定运输成本；落表时 `metric_type` 取 `UNIT` 或 `FIXED`，维护 `transport_routing_id` 与 `source_stock_point_id`，`cost_sub_category` 留空。

* **库存成本**（`cost_category = INVENTORY`）

  * **仓储成本率**：仓储费用按率计费；`cost_value` 为百分比，**语义由 `time_cost_uom` 决定**（`YEAR`=每年费率，`MONTH`=每月费率）。落表时 `metric_type = RATE`，`cost_sub_category = STORAGE`，**`time_cost_uom` 必填 `YEAR` 或 `MONTH`**（初始化默认取自全局参数「默认仓储费率时间颗粒度」）。

  * **资金成本率**：库存资金占用按率计费；`cost_value` 为百分比，语义同上。落表时 `metric_type = RATE`，`cost_sub_category = CAPITAL_OCCUPATION`，**`time_cost_uom` 必填 `YEAR` 或 `MONTH`**（初始化默认取自全局参数「默认资金费率时间颗粒度」）。

* **违约罚金**（`cost_category = PENALTY`）

  * **单位晚交罚金**：按**客户**维护晚交每件罚金；落表时 `metric_type = UNIT`，`cost_sub_category = LATE_PENALTY`，维护 `customer_id`。

  * **单位欠交罚金**：按**客户**维护欠交每件罚金；落表时 `metric_type = UNIT`，`cost_sub_category = NON_DELIVERY`，维护 `customer_id`。
...
* 如何实现可扩展？核心在于「维度固定 + 枚举扩展」这两个点：
  ...
  * 行表通过枚举来表达“是什么成本 + 针对谁”：
    ...
    * metric\_type 表示计算方式：UNIT（按件）、FIXED（按次）、RATE（按率）。cost\_sub\_category 表示业务子类型，可空，例如：STORAGE（仓储成本率）、CAPITAL\_OCCUPATION、LATE\_PENALTY、NON\_DELIVERY。库存类 RATE 行须用 **time\_cost\_uom** 标明费率是「按年」还是「按月」。将来能耗里需要“电费单价/固定电费/碳排放成本”，则 metric\_type 用 UNIT/FIXED/RATE，cost\_sub\_category 新增 ENERGY\_UNIT、ENERGY\_FIXED、CARBON 等。

    * 维度字段（standard\_resource\_id / purchase\_routing\_id / transport\_routing\_id / source\_stock\_point\_id / customer\_id）是可选的，按业务要求在对应的大类中使用。如果未来某个新类别需要新的维度，比如「生产线」维度：可以在行表中新增 production\_line\_id 列，只让 cost\_category='ENERGY' 的行填值即可。

当前阶段需支持的成本类型及其字段信息（**`time_cost_uom` 与成本明细行表字段一致，落库列名 `time_cost_uom`**）：
| cost_category | metric_type | cost_sub_category | time_cost_uom | 业务含义 |
|---------------|-------------|-------------------|---------------|----------|
| PRODUCTION | UNIT | (空) | (空) | 单位生产成本 |
| PRODUCTION | FIXED | (空) | (空) | 固定生产成本 |
| PURCHASE | UNIT | (空) | (空) | 单位采购成本 |
| PURCHASE | FIXED | (空) | (空) | 固定采购成本 |
| TRANSPORT | UNIT | (空) | (空) | 单位运输成本 |
| TRANSPORT | FIXED | (空) | (空) | 固定运输成本 |
| INVENTORY | RATE | STORAGE | `YEAR` 或 `MONTH`（必填） | 仓储成本率 |
| INVENTORY | RATE | CAPITAL_OCCUPATION | `YEAR` 或 `MONTH`（必填） | 资金成本率 |
| PENALTY | UNIT | LATE_PENALTY | (空) | 单位晚交罚金 |
| PENALTY | UNIT | NON_DELIVERY | (空) | 单位欠交罚金 |
...
* 成本度量类型（必填，枚举：单位成本/固定成本/按率计费，对应后端的 UNIT/FIXED/RATE）

* 成本子类（按需填写，枚举：仓储成本率/资金成本率/晚交罚金/欠交罚金，对应后端的 STORAGE/CAPITAL_OCCUPATION/LATE_PENALTY/NON_DELIVERY；生产/采购/运输成本可不填）
...
* 费率时间颗粒度 `time_cost_uom`（**库存成本且度量类型为 RATE** 时必填，枚举 `YEAR` / `MONTH`；其余成本类型可空）
...
- 生成成本明细时需正确写入 metric_type 与 cost_sub_category。
...
3. 查询全局参数，获取**默认仓储成本率**及**默认仓储费率时间颗粒度**

笛卡尔积，1 X 2，仓储成本率填充全局参数中的百分比，`time_cost_uom` 填充全局参数中的颗粒度，形成库存成本原始数据
...
3. 查询全局参数，获取**默认资金成本率**及**默认资金费率时间颗粒度**

笛卡尔积，1 X 2，资金成本率填充全局参数中的百分比，`time_cost_uom` 填充全局参数中的颗粒度，形成库存成本原始数据
...
3. 除了备注、单位生产成本、固定制造成本，检查当前数据是否有其它字段值为空，有一个空值就记录**错误数+1**
...
3. 若全局参数里“库存点物品的库存仓储成本=启用”，检查仓储成本率是否有空值，有一个就记录**错误数+1**

4. 若全局参数里”库存点物品的库存资金成本=启用”，检查资金成本率是否有空值，有一个就记录**错误数+1**

5. 若上述仓储/资金开关启用，还须检查对应库存成本明细行在落库时 `time_cost_uom` **是否为 `YEAR` 或 `MONTH`**（缺失或非法则**错误数+1**）

6. 除了备注、仓储成本率、资金成本率、time_cost_uom，检查当前数据是否有其它字段值为空，有一个空值就记录**错误数+1**
...
**7. WAREHOUSE_STORAGE_COST（仓储成本，基于仓储成本率）**

按库存价值与仓储成本率计费，与库存资金成本的计量结构对齐；**费率百分比的含义由该行 `time_cost_uom` 决定**。

计算公式：`仓储成本 = 平均库存价值 × 仓储成本率(%) × 时间系数`

- **平均库存价值**：与下文「库存资金成本」相同，`(期初库存 + 期末库存) / 2 × 单位价值`
- **仓储成本率(%)**：取成本明细中 `metric_type='RATE'` + `cost_sub_category='STORAGE'` 的 `cost_value`（百分比小数参与运算）
- **`time_cost_uom`（仓储）**：取上述成本明细行上的 `time_cost_uom`（须为 `YEAR` 或 `MONTH`）
- **单位价值获取优先级**：与下文「库存资金成本」一致
- **时间系数**：由**系统计划时段**与**该行 `time_cost_uom`**共同决定，将「每年费率」或「每月费率」折算到当前计划时段长度：
  - **系统时段为月**：`time_cost_uom=YEAR` → `1/12`；`MONTH` → `1`
  - **系统时段为周**：`YEAR` → `1/52`；`MONTH` → `当周覆盖的日历天数 ÷ 当周所归属自然月的日历总天数`（跨月周可按主归属月或按日拆分比例，实现须固定一种规则并与本表一致）
  - **系统时段为日**：`YEAR` → `1/365`；`MONTH` → `1/该自然月日历天数`

**8. CAPITAL_OCCUPATION_COST（库存资金成本）**

按库存价值和资金成本率计费，反映库存占用资金的机会成本；**费率语义同样由该行 `time_cost_uom` 决定**。

计算公式：`库存资金成本 = 平均库存价值 × 资金成本率(%) × 时间系数`

- **平均库存价值**：`(期初库存 + 期末库存) / 2 × 单位价值`
- **资金成本率(%)**：优先使用成本明细中 `metric_type='RATE'` + `cost_sub_category='CAPITAL_OCCUPATION'` 的 `cost_value`
- **`time_cost_uom`（资金）**：优先取该成本明细行上的 `time_cost_uom`
- **单位价值获取优先级**：
...
1. **全局参数**：「库存点物品的库存持有成本」调整为「库存点物品的仓储成本率」；去除「库存点物品的最大库存违反成本」开关；「**默认仓储成本率**」「**默认资金成本率**」为百分比（默认 0%，为 0 时建议提示配置）；各增加**费率时间颗粒度**下拉（`YEAR`/`MONTH`），与默认百分比组合表示「按年」或「按月」的费率；保留「库存点物品的库存资金成本」开关。
2. **成本明细-库存成本TAB**：展示「仓储成本率」「仓储费率颗粒度」「资金成本率」「资金费率颗粒度」；**库存成本合计（率）**仅在两行颗粒度一致时为两者百分比之和，**颗粒度不一致时显示「—」或提示不可直接相加**，**仅界面展示**（详见原型图 `财务管理.html`）
...
* **数据表格**（按 Tab 不同）：
  ...
  * **库存成本**（挂靠对象：**库存点物品**）：勾选、物品代码、库存点、版本、时段、**仓储成本率**、**仓储费率颗粒度（年/月，对应 `time_cost_uom`）**、**资金成本率**、**资金费率颗粒度**、货币、备注、操作；**库存成本合计（率）**为原型图中的只读汇总列（颗粒度一致时才数值相加）。
...
* **弹窗**：
  ...
  * 新增/编辑弹窗：按当前 Tab 显示/隐藏不同字段 —— 生产含固定生产成本，采购含固定采购成本，运输含固定运输成本，库存成本含仓储/资金成本率及各自费率颗粒度，违约罚金含物品、库存点、客户、单位晚交罚金、单位欠交罚金。
...
* **行内编辑**：
  ...
  * 单位生产成本、固定生产成本、单位采购成本、固定采购成本、单位运输成本、固定运输成本、仓储成本率、仓储费率颗粒度、资金成本率、资金费率颗粒度、单位晚交罚金、单位欠交罚金支持双击编辑（颗粒度为下拉或等效控件）。
