...
CREATE TABLE sop_pla_finance_cost_line (
  ...
  metric_type     varchar(32) NOT NULL COMMENT '度量类型: UNIT/FIXED/RATE，表示计算方式（按件/按次/按率）',
  cost_sub_category varchar(32) DEFAULT NULL COMMENT '成本子类: VIOLATION/HOLDING/CAPITAL_OCCUPATION/LATE_PENALTY/NON_DELIVERY',
...
* 如何实现可扩展：
  ...
  * 维度固定在头表，头表只负责公共维度：版本、库存点、物品、时间段、货币等。这些维度基本不会因为新增一种成本类别而改变，所以头表结构长期稳定。

  * 行表通过枚举来表达“是什么成本 + 针对谁”：

    * cost_category：表示成本大类，例如现在有：PRODUCTION / PURCHASE / TRANSPORT / INVENTORY / PENALTY。例如将来要加“能耗成本”，只需要新增一个枚举值 ENERGY 即可。

    * metric_type 表示计算方式：UNIT（按件）、FIXED（按次）、RATE（按率）。cost_sub_category 表示业务子类型，可空，例如VIOLATION、HOLDING、CAPITAL_OCCUPATION、LATE_PENALTY、NON_DELIVERY。假设将来能耗里需要“电费单价/固定电费/碳排放成本”，则 metric_type 用 UNIT/FIXED/RATE，cost_sub_category 新增 ENERGY_UNIT、ENERGY_FIXED、CARBON 等。

    * 维度字段（standard_resource_id / purchase_routing_id / transport_routing_id / source_stock_point_id / customer_id）是可选的，只在对应的大类中使用；如果未来某个新类别需要新的维度，比如「生产线」维度：可以在行表中新增 production_line_id 列，只让 cost_category='ENERGY' 的行填值即可。
...
* 成本子类（按需填写，枚举：最大库存违反/库存持有/资金占用/晚交罚金/欠交罚金，对应后端的 VIOLATION/HOLDING/CAPITAL_OCCUPATION/LATE_PENALTY/NON_DELIVERY；生产/采购/运输成本可不填）
