...
CREATE TABLE sop_pla_finance_cost_line (
  ...
  cost_sub_category varchar(32) DEFAULT NULL COMMENT '成本子类: VIOLATION/HOLDING/CAPITAL_OCCUPATION/LATE_PENALTY/NON_DELIVERY，仅部分大类需要',
...
* **生产成本** &#x20;
  ...
  * 行：`cost_category='PRODUCTION'` &#x20;
    ...
    * 单位生产成本：`metric_type='UNIT'` + `standard_resource_id`（cost_sub_category 为空）&#x20;

    * 固定生产成本：`metric_type='FIXED'` + `standard_resource_id`（cost_sub_category 为空）

* **采购成本** &#x20;

  * 行：`cost_category='PURCHASE'` &#x20;

    * 单位/固定采购成本：`metric_type='UNIT'/'FIXED'` + `purchase_routing_id`（cost_sub_category 为空）

* **运输成本** &#x20;

  * 行：`cost_category='TRANSPORT'` &#x20;

    * 单位/固定运输成本：`metric_type='UNIT'/'FIXED'` + `transport_routing_id` + `source_stock_point_id`（cost_sub_category 为空）

* **库存成本** &#x20;

  * 行：`cost_category='INVENTORY'` &#x20;

    * 单位最大库存违反成本：`metric_type='UNIT'` + `cost_sub_category='VIOLATION'` &#x20;

    * 库存持有成本：`metric_type='UNIT'` + `cost_sub_category='HOLDING'`

    * 资金占用成本率（年化）：`metric_type='RATE'` + `cost_sub_category='CAPITAL_OCCUPATION'`

* **违约罚金** &#x20;

  * 行：`cost_category='PENALTY'` &#x20;

    * 单位晚交罚金：`metric_type='UNIT'` + `cost_sub_category='LATE_PENALTY'` + `customer_id` &#x20;

    * 单位欠交罚金：`metric_type='UNIT'` + `cost_sub_category='NON_DELIVERY'` + `customer_id`
...
* 如何实现可扩展？核心在于「维度固定 + 枚举扩展」这两个点：
  ...
  * 行表通过枚举来表达“是什么成本 + 针对谁”：
    ...
    * metric\_type 表示计算方式：UNIT（按件）、FIXED（按次）、RATE（按率）。cost\_sub\_category 表示业务子类型，可空，例如：VIOLATION、HOLDING、CAPITAL\_OCCUPATION、LATE\_PENALTY、NON\_DELIVERY。将来能耗里需要“电费单价/固定电费/碳排放成本”，则 metric\_type 用 UNIT/FIXED/RATE，cost\_sub\_category 新增 ENERGY\_UNIT、ENERGY\_FIXED、CARBON 等。
